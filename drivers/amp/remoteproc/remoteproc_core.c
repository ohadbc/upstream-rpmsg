...
/**
 * rproc_handle_carveout() - handle contig memory allocation requests
 * @rproc: rproc handle
 ...
 * needed to map it (in case @rproc is using an IOMMU).
 *
 * The latter is extrememly important because a simple and seemingly benign
 * matter such as order and size of each iommu mapping may greatly differ
 * in the number of required TLB entries, which may have substantial impact
 * on performance (in case we run out of TLB space).
 */
static int rproc_handle_carveout(struct rproc *rproc, struct fw_resource *rsc)
{
	 ...
	 * firmware was compiled with and/or linked against.
	 *
 ...
 * possible handlers: one is invoked upon registeration of @rproc,
 ...
 *
 * Note: this function is called synchronously while booting a remote
 * processor.
 */
 ...
 * of registreing this remote processor, additional virtio drivers will be
 * probed.
 *
