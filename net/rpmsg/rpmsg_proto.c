 ...
 * them with its destination addresses (so sockaddr_rpmsg lookups are quick).
 *
							...
							int alen, int flags)
{
	...
	if (sk->sk_state != RPMSG_OPEN && sk->sk_state != RPMSG_BOUND)
		return -EBADFD;
...
static void __devexit rpmsg_proto_remove(struct rpmsg_channel *rpdev)
{
	...
	dev_info(dev, "rpmsg proto channel is removed\n");

	mutex_lock(&rpmsg_channels_lock);

	if (!radix_tree_delete(&rpmsg_channels, dst))
